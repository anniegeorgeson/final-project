...
Histogram to visualize the new column:

```{r}
ggplot(food_census_join, aes(x=percent_food_insecure)) +
  geom_histogram(bins=50)
# can take this out at the end if it seems redundant - i added a plot below this so this can probably go
```
...
#note that the percentage is somewhat overstated since the income bracket is from 15,000 to 24,999 
...
Running a regression model with one variable: 

```{r}
single_variable_model = lm(percent_food_insecure~percent_white_population, food_census_join)
#food insecurity is being treated as the dependent variable while everything that comes after the ~ is independent 
...
Visualization of the one variable regression model: 

```{r}
ggplot(food_census_join, aes(x=percent_white_population, y=percent_food_insecure)) +
  geom_point(size=0.5) +
  geom_abline(intercept=24.17790, slope=-0.07591, color="red")

#cut this too probably
```
